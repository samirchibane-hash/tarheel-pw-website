const LocationApp = () => {
  ...
  React.useEffect(() => {
    ...
    Object.entries({ "--bg": c.bg, "--bg-2": c.bg2, "--paper": c.paper, "--ink": c.ink, "--ink-2": c.ink2, "--ink-3": c.ink3, "--rule": c.rule })
      .forEach(([k, v]) => root.style.setProperty(k, v));

    root.style.setProperty("--f-display", {
      fraunces: '"Fraunces", Georgia, serif',
      ibmplex: '"IBM Plex Serif", Georgia, serif',
      instrument: '"Instrument Serif", Georgia, serif',
      inter: '"Inter", -apple-system, sans-serif',
    }[tweaks.display] || '"Fraunces", Georgia, serif');

    const l = window.__LOCATION__;
    if (l) {
      document.title = "Water Filtration " + l.city + ", " + l.state + " | Tarheel Pure Water";
    }
  }, [tweaks]);
...
ReactDOM.createRoot(document.getElementById("root")).render(<LocationApp />);
